...
public abstract class Game {
    ...
    Game(PlayableDeck playerDeck, PlayableDeck opponentDeck) {
    ...
    public Game() {
        ...
        for (int i = 0; i < 30; i++) {
            tempSet = new HashSet<>();
            playerDeck.addCard(new CardSpell("Id" + i, "naam" + i, "cardType1", 1, "heroType1",
                    "heroType", "http://media.services.zam.com/v1/media/byName/hs/cards/enus/EX1_015.png",
                    "", tempSet, 2, 1, "", "", true,
                    1, true, "", true));

            tempSet = new HashSet<>();
            opponentDeck.addCard(new CardMinion("ID" + i, "name" + i, "cardType2", i / 6 + 1,
                    "HeroType", "info", "ig", i / 5 + 1, (i + 1) / 5 + 1,
                    "race", "mechanics", tempSet));
        }

        final HeroPower playerHeroPower = new HeroPower("", 0, "", heal, 1, 1,
                "", true);
        final HeroPower opponentHeroPower = new HeroPower("", 0, "", attack, 1,
                1, "", true);

        final Hero playerHero = new Hero("heroPlayer", "img", playerHeroPower);
        final Hero opponentHero = new Hero("heroOpponent", "img1", opponentHeroPower);
    ...
    // does spell-action
    private void spell(Set<CardAbility> abilities, CardSpell card) {
    ...
    void addHealthHero(int healAmount) {
        pf.getCurrentPlayer().getHero().incrHealth(healAmount);
    }

    void addHealthCard(int healAmount, String id) {
        final Player curPlayer = pf.getCurrentPlayer();
